...
namespace Etherna.BeeNet.Models
{
    ...
    public class PostageBuckets
    {
            ...
            uint[]? initialBuckets = null)
        {
            ...
            _buckets = BucketsToDictionary(initialBuckets ?? []);
        }
        ...
        // Helpers.
        private static ConcurrentDictionary<uint, uint> BucketsToDictionary(uint[] buckets) =>
            new(buckets.Select((c, i) => (c, (uint)i))
                .ToDictionary<(uint value, uint index), uint, uint>(pair => pair.index, pair => pair.value));

        private static uint[] DictionaryToBuckets(ConcurrentDictionary<uint, uint> dictionary)
        {
