    ...
    public class SwarmEpochFeedTest
    {
        ...
        public static IEnumerable<object[]> FindLastEpochChunkBeforeDateTests
        {
            get
            {
                ...
                // Starting chunk epoch index is at max resolution (level 0).
                {
                    ...
                    var startingChunkTimestamp = new byte[] { 0, 0, 0, 1, 42, 5, 242, 0 }; //1970-01-01 00:00:05
                ...
                // Chunk with child epoch at date is valid and at max resolution.
                {
                    ...
                    var startingChunkReference = SwarmFeedChunk.BuildHash(ChunkOwner, ChunkTopic, startingEpochIndex, new Hasher());
                    var startingChunkTimestamp = new byte[] { 0, 0, 0, 0, 238, 107, 40, 0 }; //1970-01-01 00:00:04
                    var startingChunkPayload = startingChunkTimestamp.Concat(new byte[] { 1, 2, 3 }).ToArray(); //arbitrary content payload
                    var startingChunk = new SwarmFeedChunk(startingEpochIndex, startingChunkPayload, startingChunkReference);
                    ...
                    var childChunkTimestamp = new byte[] { 0, 0, 0, 1, 42, 5, 242, 0 }; //1970-01-01 00:00:05
                    ...
                    tests.Add(new FindLastEpochChunkBeforeDateTestElement(
                        ...
                        gateMock => gateMock.Setup(g => g.TryGetAsync(
                                childChunkReference,
                                It.IsAny<bool>()))
                            .ReturnsAsync(new SwarmChunk(childChunkReference, childChunkPayload)),
                        gateMock => gateMock.Verify(g => g.TryGetAsync(
                            It.IsAny<SwarmHash>(),
                ...
                // Chunk with left brother of child epoch at date is valid and at max resolution.
                {
                    ...
                    var leftChildChunkTimestamp = new byte[] { 0, 0, 0, 0, 238, 107, 40, 0 }; //1970-01-01 00:00:04
                    ...
                    tests.Add(new FindLastEpochChunkBeforeDateTestElement(
                        ...
                        gateMock =>
                        {
                            ...
                            gateMock.Setup(g => g.TryGetAsync(
                                    ...
                                    It.IsAny<bool>()))
                                .ReturnsAsync(new SwarmChunk(leftChildChunkReference, leftChildChunkPayload));
                        },
                        gateMock => gateMock.Verify(g => g.TryGetAsync(
                            It.IsAny<SwarmHash>(),
                ...
                // Chunk on child at date (right) is successive to date, and chunk on its left brother epoch doesn't exist.
                {
                    ...
                    var rightChildEpochIndex = new SwarmEpochFeedIndex(6, 1, new Hasher());
                    var rightChildChunkReference = SwarmFeedChunk.BuildHash(ChunkOwner, ChunkTopic, rightChildEpochIndex, new Hasher());
                    var rightChildChunkTimestamp = new byte[] { 0, 0, 0, 1, 161, 59, 134, 0 }; //1970-01-01 00:00:07
                    ...
                    tests.Add(new FindLastEpochChunkBeforeDateTestElement(
                        ...
                        gateMock =>
                        {
                            gateMock.Setup(g => g.TryGetAsync(
                                    rightChildChunkReference,
                                    It.IsAny<bool>()))
                                .ReturnsAsync(new SwarmChunk(rightChildChunkReference, rightChildChunkPayload));
                            gateMock.Setup(g => g.TryGetAsync(
                                    leftChildChunkReference,
                                    It.IsAny<bool>()))
                                .ReturnsAsync((SwarmChunk?)null);
        ...
        public static IEnumerable<object[]> TryFindStartingEpochChunkOnlineTests
        {
            get
            {
                ...
                {
                    var startingEpochIndex = new SwarmEpochFeedIndex(4, 2, new Hasher());
                    var chunkTimestamp = new byte[] { 0, 0, 0, 1, 42, 5, 242, 0 }; //1970-01-01 00:00:05
                    ...
                    tests.Add(new TryFindStartingEpochChunkOnlineTestElement(
                        ...
                        gateMock => gateMock.Setup(g => g.TryGetAsync(
                                reference,
                                It.IsAny<bool>()))
                            .ReturnsAsync(new SwarmChunk(reference, payload)),
                        gateMock => gateMock.Verify(g => g.TryGetAsync(
                            It.IsAny<SwarmHash>(),
                            It.IsAny<bool>()), Times.Once()),
                ...
                // Chunk exists but with successive date and index was left at max level.
                {
                    ...
                    var chunkTimestamp = new byte[] { 0, 0, 0, 1, 161, 59, 134, 0 }; //1970-01-01 00:00:07
                    var payload = chunkTimestamp.Concat(new byte[] { 1, 2, 3 }).ToArray(); //arbitrary content payload
                    var reference = SwarmFeedChunk.BuildHash(ChunkOwner, ChunkTopic, startingEpochIndex, new Hasher());
                ...
                // Valid chunk is found at left, starting chunk is not found.
                {
                    ...
                    var leftChunkTimestamp = new byte[] { 0, 0, 0, 1, 42, 5, 242, 0 }; //1970-01-01 00:00:05
                    ...
                    tests.Add(new TryFindStartingEpochChunkOnlineTestElement(
                        ...
                        gateMock =>
                        {
                            ...
                            gateMock.Setup(g => g.TryGetAsync(
                                    leftChunkReference,
                                    It.IsAny<bool>()))
                                .ReturnsAsync(new SwarmChunk(leftChunkReference, leftChunkPayload));
                        },
                        gateMock => gateMock.Verify(g => g.TryGetAsync(
                            It.IsAny<SwarmHash>(),
                            It.IsAny<bool>()), Times.Exactly(2)),
                ...
                // Valid chunk is found at left, starting chunk is successive to date.
                {
                    var startingEpochIndex = new SwarmEpochFeedIndex(6, 1, new Hasher());
                    var startingChunkReference = SwarmFeedChunk.BuildHash(ChunkOwner, ChunkTopic, startingEpochIndex, new Hasher());
                    var startingChunkTimestamp = new byte[] { 0, 0, 0, 1, 161, 59, 134, 0 }; //1970-01-01 00:00:07
                    var startingChunkPayload = startingChunkTimestamp.Concat(new byte[] { 1, 2, 3 }).ToArray(); //arbitrary content payload
                    ...
                    tests.Add(new TryFindStartingEpochChunkOnlineTestElement(
                        ...
                        gateMock =>
                        {
                            gateMock.Setup(g => g.TryGetAsync(
                                    startingChunkReference,
                                    It.IsAny<bool>()))
                                .ReturnsAsync(new SwarmChunk(startingChunkReference, startingChunkPayload));
                            gateMock.Setup(g => g.TryGetAsync(
                                    leftChunkReference,
                                    It.IsAny<bool>()))
                                .ReturnsAsync(new SwarmChunk(leftChunkReference, leftChunkPayload));
                ...
                // Chunk valid is found at parent, starting chunk is not found.
                {
                    ...
                    var parentChunkTimestamp = new byte[] { 0, 0, 0, 1, 42, 5, 242, 0 }; //1970-01-01 00:00:05
                    ...
                    tests.Add(new TryFindStartingEpochChunkOnlineTestElement(
                        ...
                        gateMock =>
                        {
                            ...
                            gateMock.Setup(g => g.TryGetAsync(
                                    ...
                                    It.IsAny<bool>()))
                                .ReturnsAsync(new SwarmChunk(parentChunkReference, parentChunkPayload));
                        },
                        gateMock => gateMock.Verify(g => g.TryGetAsync(
                            It.IsAny<SwarmHash>(),
                            It.IsAny<bool>()), Times.Exactly(2)),
        ...
        public async Task TryGetFeedChunkAsync(bool chunkExists)
        {
            ...
            if (chunkExists)
                chunkStoreMock.Setup(c => c.TryGetAsync(
                        referenceHash,
                        It.IsAny<bool>()))
                    .ReturnsAsync(new SwarmChunk(referenceHash, data));
            else
                chunkStoreMock.Setup(c => c.TryGetAsync(
                        referenceHash,
                        It.IsAny<bool>()))
