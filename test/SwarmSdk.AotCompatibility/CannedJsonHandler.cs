...
namespace Etherna.SwarmSdk.AotCompatibility
{
    ...
    // Test message handler that returns the same canned JSON body for every request, so the probe can
    // drive a client response path without a live node.
    internal sealed class CannedJsonHandler(string json) : HttpMessageHandler
    {
        // Methods.
